<template>
  <div class="hello">
    <vue-headful title="RG EVA 00 新世紀福音戰士 零號機[素組]"/>
    <h1>{{ msg }}</h1>
...
export default {
  ...
  data () {
    return {
      msg: 'RG EVA 00 新世紀福音戰士 零號機[素組]',
      tools: [
        {src: 'unnamed (11).jpg', name: '模型本體', des: '身形的比例抓得很好看,但是拘束器(外甲)底下機械感有點太重,不太符合EVA原始設定的"生物感"。'},
        {src: 'unnamed (7).jpg', name: '頭部透明件', des: '頭部的綠色透明件是直接裝在白色零件上面的,透過透明綠看到的白色感覺非常廉價,所以建議在會被透明綠蓋住的白色部分上一層銀色,效果會好很多'},
        {src: 'unnamed (7).jpg', name: '可動性', des: '這款套件提供了兩種型態的零號機來做零件上的替換,分別是"零號機"跟"零號機改",兩者的差異就只有在胸口跟肩膀而已。'}
      ]
    }
  }
}
